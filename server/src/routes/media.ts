...
mediaRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    let saved: { url: string; publicId?: string; provider: 'cloudinary' | 'local'; width?: number; height?: number };

    if (env.mediaProvider === 'cloudinary') {
      const result = await new Promise<any>((resolve, reject) => {
        cloudinary.uploader
          .upload_stream(
            { folder: 'our-little-world', resource_type: isImage ? 'image' : 'auto' },
      ...
      saved = { url: `/uploads/${name}`, publicId: name, provider: 'local' };
    }
